...
Step 2. Grep upstream:
          rg 'shfl\.sync\.bfly' corpus/nvidia/cuda-official/cuda-toolkit-documentation-13.2/ptx-isa/
Step 3. (Optional) Sanity-check against the canonical baseline:
          kp_introspect microbench compute-latency --instructions shfl.sync.bfly --samples 4096 \
              --output sources/experience/hw-probes/canonical/shfl-sync-bfly.json
...
Step 8. Write the probe record to
          sources/experience/hw-probes/<instruction-slug>/<YYYY-MM-DD>-<task-slug>.md
...
Step 3. (Optional) Sanity-check against canonical:
          kp_introspect microbench memory-latency --levels <level> --samples 4096 \
              --output sources/experience/hw-probes/canonical/memory-<level>.json
...
Step 8. Write the probe record to
          sources/experience/hw-probes/<level-slug>/<YYYY-MM-DD>-<task-slug>.md
...
| Bandwidth or latency of a feature the whitepaper announces (TMA, DSMEM cluster, wgmma) | Record `status: blocked` with reference to `reasoning/hardware-microbench.md` §deferred — MVP does not execute feature sweeps |
