...
target_path: 30-skill/compute/warp-primitives/   # relative to knowledge/, ends in / for dir or .md for file
hardware:
  device: H200
  sm: "9.0a"
  cuda_runtime: "12.4"
upstream_scope:                              # absolute paths OR knowledge/-relative paths under 05-source-corpus/; agent forbidden to grep outside
  - 05-source-corpus/cuda-official/cuda-toolkit-documentation-13.2/   # knowledge/-relative entry under 05-source-corpus is allowed
...
success_criteria:                            # self-checked before done-report (MVP rules)
  ...
  - lint_clean: true
  - at_least_one_hw_probe_linked: true       # skill.md's ## Measured Characteristics must link ≥1 record under 80-experience/hw-probes/
...
| --- | --- | --- |
| `build-skill` | Produce a skill four-pack under `30-skill/<family>/<skill>/` | `skill.md` (REQUIRED) + `pitfalls.md` (REQUIRED) + `apis.md` (OPTIONAL — when the skill touches named APIs). `verified.md` is dropped in MVP; do not create it. |
| `probe-api` | Run an F10 probe on one API and back-fill `10-api-raw/` | `80-experience/api-probes/...` + updated `10-api-raw/...` |
| `build-pattern` | Produce a pattern four-pack under `20-pattern/cuda-core/<op>/` | `INDEX.md` + `ROUTING.md` + `library-fallback.md` + `TASK-PACKET.md` |
...
| `build-hardware-feature` | Produce a hardware-feature four-pack under `40-hardware-feature/<feature>/` (TMA, wgmma, DSMEM, cluster-launch-control, …). `target_path` is the feature directory; companion outputs (code skeleton, hw-probe record) live outside that directory and MUST be listed under `also_writes:`. | `skill.md` + `pitfalls.md` (under `target_path`); `*.cu` + `build.sh` under `60-code/<source-repo>/<feature>/`; `<YYYY-MM-DD>-<slug>.md` under `80-experience/hw-probes/<feature>/` |
| `extend-hardware-feature` | Append measured material to an existing `40-hardware-feature/<feature>/skill.md` (typically a shape sweep or follow-up probe) without changing the skill's identity. `target_path` MUST be the existing feature directory; new probe records under `80-experience/hw-probes/<feature>/` are listed in `also_writes:`. | Append `## Shape-selection guide` (or analogous section) to the existing `skill.md`; new `<YYYY-MM-DD>-<slug>.md` under `80-experience/hw-probes/<feature>/` |
| `build-classical-algo` | Produce a classical-algorithm four-pack under `50-classical-algo/<algo>/` (warp-specialization, persistent-kernel, online-softmax, split-k, …). The algorithm code skeleton lives separately under `60-code/<source-repo>/<algo>/`; the A/B ablation record lives under `80-experience/api-probes/<op>/`. | `skill.md` + `pitfalls.md` (under `target_path`); `*.cu` + `build.sh` under `60-code/<source-repo>/<algo>/`; `<YYYY-MM-DD>-<slug>-ablation.md` under `80-experience/api-probes/<op>/` |
...
2. **All non-`target_path` outputs are listed under `also_writes:`**. The packet enumerates the exact directories the agent is allowed to touch (e.g. `60-code/<source-repo>/<topic>/`, `80-experience/hw-probes/<feature>/`, `80-experience/api-probes/<op>/`).
...
If a task needs to land in one of two possible directories (e.g. AC-7 of the cutlass plan: `30-skill/compute/gemm-fused/` vs. `20-pattern/fused-cuda-tensor-core/`), the task packet must commit to one before execution; the rationale goes in the per-task report.
...
prior_art:                                           # existing entries to extend or supersede
  - path: 30-skill/compute/warp-primitives/skill.md
...
- **`upstream_scope`** — list of absolute paths or `knowledge/`-relative paths under `05-source-corpus/`. Agent uses these for `grep_source` / `read_file` on upstream material. Grepping outside this list is forbidden; reading `knowledge/` and `tools/` is always allowed (for reading meta-skills and invoking the CLI tools).
...
target_path: 30-skill/compute/warp-primitives/
hardware:
  device: H200
  sm: "9.0a"
  cuda_runtime: "12.4"
upstream_scope:
  - 05-source-corpus/cuda-official/cuda-toolkit-documentation-13.2/programming-guide/
  - 05-source-corpus/cuda-official/cuda-toolkit-documentation-13.2/ptx-isa/
  - {{CCCL_REPO_REF}}/cub/
...
2. Resolve `target_path` → absolute path under `knowledge/`. Refuse if the path escapes `knowledge/`.
