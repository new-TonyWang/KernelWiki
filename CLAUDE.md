...
A structured, multi-vendor knowledge base of GPU kernel optimization, optimized for LLM agent retrieval. Currently covering NVIDIA Blackwell (SM100) and Hopper (SM90), with schema ready for Huawei Ascend and Biren.

## Navigation

### Recommended: use the skill scripts
```bash
python3 scripts/query.py "<keywords>" [--tag X --type Y --vendor nvidia]
...
8. **By vendor**: Read `queries/by-vendor.md` → find vendor → see all pages per type
...
### Layer 2: Wiki (`wiki/`)
Synthesized knowledge pages, organized by vendor. Cross-referenced by `id`.
- `wiki/nvidia/hardware/` — Hardware feature pages (tcgen05, TMEM, CLC, TMA, etc.)
- `wiki/nvidia/techniques/` — Optimization techniques (warp specialization, pipelining, etc.)
- `wiki/nvidia/patterns/` — Problem → solution diagnosis (low SM util → CLC, etc.)
- `wiki/nvidia/kernels/` — Kernel case studies (FlashAttention-4, DeepGEMM, etc.)
- `wiki/nvidia/languages/` — DSL/language guides (CuTe DSL, Triton, CUDA C++, PTX)
- `wiki/nvidia/migration/` — Hopper → Blackwell migration patterns
- `wiki/nvidia/foundations/` — Foundational CUDA optimization skills (coalescing, warp primitives, etc.)
- `wiki/nvidia/operator-routing/` — Operator decision trees (elementwise, reduction, GEMM, etc.)
- `wiki/nvidia/api-definitions/` — API definitions (CUDA Runtime, PTX instructions)
- `wiki/nvidia/code-walkthroughs/` — Code repository extractions (CUTLASS, FlashAttention)
...
| source-experience | exp- | title, type=experience, vendor, probe_slug, evidence_level, measured_on |
...
| wiki-operator-routing | routing- | title, type=operator-routing, vendor, operator |
| wiki-algorithm | algo- | title, type=algorithm, vendor, tags, evidence_level, source |
| wiki-code-walkthrough | code- | title, type=code-walkthrough, vendor, upstream_repo |
| wiki-pitfall | pitfall- | title, type=pitfall, vendor |
