...
// Each lane reads from (lane - delta). Lanes with lane < delta get their own
// value back (per CUDA Programming Guide: the source lane ID does not wrap,
// so the lower `delta` lanes remain unchanged).
...
static void ref_up(const float* in, float* out, int n, int delta) {
    for (int i = 0; i < n; i++) {
        int warp_base = (i / WARP) * WARP;
        int lane = i & (WARP - 1);
        int src_lane = lane - delta;
        if (src_lane < 0) out[i] = in[i];                    // own value
...
int main() {
    ...
    for (int i = 0; i < 5; i++) shfl_up_kernel<<<grid, BLK>>>(d_in, d1, d4, d16, N);
    ...
    for (int i = 0; i < 20; i++) {
        cudaEventRecord(t0);
        shfl_up_kernel<<<grid, BLK>>>(d_in, d1, d4, d16, N);
