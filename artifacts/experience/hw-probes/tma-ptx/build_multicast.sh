#!/usr/bin/env bash
# Build the cutlass-free TMA cluster-multicast probe + verify cutlass-free gates.
set -euo pipefail
SRC=tma_multicast_probe.cu
BIN=tma_multicast_probe
...
[ "$PRE" = "0" ] && echo "PASS: 0 cutlass:: / cute:: matches." || { echo "FAIL: $PRE."; exit 1; }
...
SYM=$(/usr/local/cuda-12.9/bin/cuobjdump --dump-elf-symbols "$BIN" 2>/dev/null \
        | grep -cE 'cutlass::|cute::' || true)
[ "$SYM" = "0" ] && echo "PASS: 0 cutlass:: / cute:: symbols." || { echo "FAIL: $SYM."; exit 1; }
