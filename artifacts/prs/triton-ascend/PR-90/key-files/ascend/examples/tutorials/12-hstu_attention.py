...
def _hstu_attn_fwd_compute(  # noqa C901
    ...
    seq_start = seq_start.to(tl.int64)
    ...
    if HAS_BIAS:
        bias_block_ptr = tl.make_block_ptr(
            base=bias + off_z * H * MAX_SEQ_LEN * MAX_SEQ_LEN + off_h * MAX_SEQ_LEN * MAX_SEQ_LEN,
...
def _hstu_attn_fwd(  # noqa C901
    ...
    for col in range(core_id, tasks, CORE_NUM):
        ...
        start_m = col * BLOCK_M
        while start_m >= seq_end * H // 2:
            cur_batch += 1
            seq_end = tl.load(seq_offsets + cur_batch + 1)
        seq_start = tl.load(seq_offsets + cur_batch)
        seq_len = seq_end - seq_start
        off_z = cur_batch
        off_h = (start_m - seq_start * H // 2) // (seq_len // 2)
        start_m_1 = (start_m - seq_start * H // 2) % (seq_len // 2)
...
def _hstu_attn_bwd(  # noqa C901
    ...
    off_bias = off_z * H * MAX_SEQ_LEN * MAX_SEQ_LEN + off_h * MAX_SEQ_LEN * MAX_SEQ_LEN
    ...
    for start_n in range(0, seq_len, BLOCK_N):
        _hstu_attn_bwd_one_col_block(
            ...
            BLOCK_N=BLOCK_N,
            UNROLL=UNROLL,
            bias=bias + off_bias if HAS_BIAS else bias,
        )
...
def test_bwd(batch_size, max_seq_len, num_heads, attention_dim, data_type):
    ...
    # triton 输出
    seq_offsets = torch.tensor(seq_offset, dtype=torch.int64, device=DEVICE)
    dq = torch.zeros_like(q).npu()
    dk = torch.zeros_like(k).npu()
    dv = torch.zeros_like(v).npu()
