...
artifacts:
  ...
  profile: 80-experience/api-probes/gemm/artifacts/profiles/2026-04-28-tma-counters.csv
...
Full probe record: [80-experience/api-probes/gemm/2026-04-28-tma-bandwidth-counters.md](../../80-experience/api-probes/gemm/2026-04-28-tma-bandwidth-counters.md).

For the TMA primitive in isolation (single-tile correctness + 16-config DRAM-bound bandwidth sweep, peak 3.72 TB/s = 77.5 % HBM3e on H200), see the cutlass-free hw-probes [80-experience/hw-probes/tma-ptx/2026-04-28-tma-ptx-hello.md](../../80-experience/hw-probes/tma-ptx/2026-04-28-tma-ptx-hello.md) and [80-experience/hw-probes/tma-ptx/2026-04-29-tma-throughput.md](../../80-experience/hw-probes/tma-ptx/2026-04-29-tma-throughput.md).

## Minimum repro

`80-experience/api-probes/gemm/artifacts/gemm_compare.cu` (vendored from cutlass example 48 at commit `f74fea9c`), plus `build.sh` (nvcc invocation), `run.sh` (build → run → ncu CSV → compute-sanitizer), and `device.json` (H200 nvidia-smi snapshot) in the same `artifacts/` directory. Build requires the cutlass include tree at the same pin and CUDA 12.9 on H200.

## How it connects to the rest of the KB

- Pairs with `40-hardware-feature/wgmma/skill.md` — the same kernel exercises both. TMA loads the operands; wgmma consumes them.
- Used by `50-classical-algo/warp-specialization/` — warp-specialization without TMA defeats the purpose, since the producer warpgroup needs a non-blocking load primitive.
- Pre-condition for `30-skill/compute/gemm/aligned/`.
