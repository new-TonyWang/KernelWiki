...
TMA + wgmma encodings live in `sm_90a` (the "a" = architecture-specific extension). Compiling with plain `sm_90` produces `ptxas` errors on `cp.async.bulk.tensor.*`. The canonical build script at `80-experience/api-probes/gemm/artifacts/build.sh` pins `-arch=sm_90a` for this reason.
