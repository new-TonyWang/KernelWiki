...
---
# 60-code/cutlass-cute/gemm-fused — fused-GEMM library usage

Library-usage knowledge for cutlass's fused-GEMM patterns (epilogue + prologue fusion). The canonical reproducible artifacts:

- Reference build of upstream `examples/50_hopper_gemm_with_epilogue_swizzle/` lives at `80-experience/api-probes/gemm/artifacts/` (built via the run scripts there); the upstream `.cu` is **not** vendored — it stays in the cutlass tree at the pinned commit.
- Custom epilogue probe: `80-experience/api-probes/gemm/artifacts/gemm_compare_relu.cu` + `relu_kernel.cu` (fused vs non-fused ReLU A/B).
...
For epilogues that *are* in cuBLASLt's catalogue (`CUBLASLT_EPILOGUE_BIAS / RELU / GELU / DRELU_BGRAD / ...`) library-first is the default — see `20-pattern/tensor-core/gemm/library-fallback.md`.
...
| Activation epilogue (ReLU / GELU) | thread functor `cutlass::epilogue::thread::LinearCombinationRelu` | any sm_90 mainloop | `LinearCombinationRelu` thread functor | Probe at `80-experience/api-probes/gemm/artifacts/gemm_compare_relu.cu` |
...
- ReLU fusion probe: `80-experience/api-probes/gemm/2026-04-28-gemm-fused.md`
- Library-first decision: `20-pattern/tensor-core/gemm/library-fallback.md` §3 cuBLASLt
