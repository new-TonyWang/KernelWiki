...
auditor_inputs:
  - wiki/nvidia/hardware/
  - wiki/nvidia/foundations/compute/
  - wiki/nvidia/foundations/memory/
  - wiki/nvidia/foundations/sync/
  - wiki/nvidia/foundations/meta/
  - wiki/nvidia/techniques/
...
This file is the deliverable for the gap-audit task in `docs/cutlass_explore_update_01.md`. It catalogs every existing GEMM/wgmma/TMA/attention-relevant entry in ``, classifies each, and lists concrete additions needed. All "missing | partial" rows cite a specific cutlass source path under `{{CUTLASS_REPO_REF}}/` that justifies the gap, and propose a target KB layer path.

The classification rubric is:
- **complete** — entry exists, satisfies its layer's contract (skill+pitfalls+optional apis for skill layer; README+tuning+optional skeleton for `wiki/nvidia/code-walkthroughs/`), and its referenced sources/artifacts are present.
...
All paths are relative to `` unless prefixed with `<path-removed> (absolute upstream paths).

---

## Section A — `wiki/nvidia/hardware/` (Hardware primitives)

| Entry | Files | Status | Evidence | Classification | Notes / Gap |
|-------|-------|--------|----------|----------------|-------------|
| `wiki/nvidia/hardware/tma/` | skill.md, pitfalls.md | verified | measured | complete | TMA cute end-to-end on Hopper; pitfalls list 8 items; could add a measured 3D TMA case but not blocking. |
| `wiki/nvidia/hardware/tma-ptx/` | skill.md, pitfalls.md | verified | measured | complete | Raw PTX TMA (cutlass-free); has 4 hw-probe records (hello, throughput, multicast v1, multicast v2). |
| `wiki/nvidia/hardware/wgmma/` | skill.md, pitfalls.md | verified | measured | complete | **Round 1 closed**: Added "## Atomic Usage (cute)" section with 11-row mapping table (MMA_Atom → PTX mnemonic → probe config → probe record) plus cute-API snippet. Cutlass anchor: `{{CUTLASS_REPO_REF}}/include/cute/atom/mma_traits_sm90_gmma.hpp`. |
| `wiki/nvidia/hardware/wgmma-ptx/` | skill.md, pitfalls.md, atomic_skeleton.md | verified | measured | complete | **Round 1 closed**: Added "## Atomic Usage (PTX)" section with raw PTX template + build/run commands, and created `atomic_skeleton.md` documenting the full single-atom sequence (fence→mma→commit→wait) with descriptor construction and fragment layout. |
| `wiki/nvidia/hardware/mma-sync-ptx/` | skill.md, pitfalls.md | draft | spec | complete | **Round 1 closed**: Created skill.md with full atom shape table (12 families), PTX mnemonic template, and 5 pitfalls. `status: draft`, `evidence_level: spec`, `requires_sm: ">=8.0"`. |
| `wiki/nvidia/hardware/ldmatrix-ptx/` | skill.md, pitfalls.md | draft | spec | complete | **Round 1 closed**: Created skill.md with all 6 ldmatrix variants (x1/x2/x4 non-transposed + x2/x4/x8 transposed) plus movmatrix, and 5 pitfalls. Uses corrected paths (`copy_traits_sm75.hpp`, not `mma_traits_sm80*.hpp`). |
| `wiki/nvidia/hardware/tcgen05-ptx/` | skill.md, pitfalls.md | draft | spec | complete | **Round 1 closed**: Created skill.md with `not_measured_on_target: true`, `requires_sm: ">=10.0"`. Documents tcgen05.cp, tcgen05.ld, and SM100_MMA_* traits. Cites Blackwell whitepaper and cutlass sm100 headers. 3 pitfalls. |

---

## Section B — `wiki/nvidia/foundations/compute/gemm*` and `wiki/nvidia/foundations/compute/attention/` (Operator skills)

| Entry | Files | Status | Evidence | Classification | Notes / Gap |
|-------|-------|--------|----------|----------------|-------------|
| `wiki/nvidia/foundations/compute/gemm/aligned/` | skill.md, pitfalls.md, tuning.md | verified | measured | complete | **Round 1 closed**: Created sibling `tuning.md` with compile-time and runtime parameter tables. Added back-link from `skill.md` body. |
| `wiki/nvidia/foundations/compute/gemm/non-aligned-tail/` | skill.md, pitfalls.md, tuning.md | partial | measured | partial | **Round 1 partial**: Created sibling `tuning.md` with tail-strategy knobs. Added back-link from `skill.md`. Status remains partial (coverage incomplete). |
| `wiki/nvidia/foundations/compute/gemm-fused/cutlass-epilogue-prologue/` | skill.md, pitfalls.md, tuning.md | partial | measured | partial | **Round 1 partial**: Created sibling `tuning.md` with EVT and prologue fusion knobs. Added back-link from `skill.md`. Status remains partial (prologue coverage incomplete). |
| `wiki/nvidia/foundations/compute/gemm-ptx/` | skill.md, pitfalls.md | partial | measured | partial | Cutlass-free PTX GEMM track. No apis.md (acceptable — pure PTX). Cross-link gap to any tuning.md is absent because no `wiki/nvidia/code-walkthroughs/.../gemm-ptx/` partner exists. Out of plan scope unless the Round-N tuning sweep targets PTX-GEMM. |
| `wiki/nvidia/foundations/compute/attention/mvp-minimal/` | skill.md, pitfalls.md, tuning.md | verified | measured | complete | **Round 9 verified**: TMA+wgmma kernel passes correctness on H200 (max_abs_err=0.000061, 0/16384 mismatches, sanitizer 0 errors). Uses canonical CUTLASS smem layout (Layout_K_SW128_Atom + Swizzle<3,4,3> + CU_TENSOR_MAP_SWIZZLE_128B). Evidence logs checked in. |
| `wiki/nvidia/foundations/compute/attention/cutlass-fmha/` | skill.md, pitfalls.md, tuning.md | verified | measured | complete | **Round 13 measured**: H200-verified via repo-local wrapper. Example: 376.1 TFLOPS (B2 H16 S1024 D128 full). Collective: 348.5 TFLOPS (B2 H16 S2048 D128 causal). |
...
## Section C — `wiki/nvidia/techniques/` (End-to-end algorithm patterns)
...
| `wiki/nvidia/code-walkthroughs/cutlass-cute/wgmma-atom-decoding/` | README.md, wgmma_skeleton.md | complete | Skeleton walkthrough for wgmma atom decoding; pairs with `wiki/nvidia/hardware/wgmma/`. |
| `wiki/nvidia/code-walkthroughs/cutlass-cute/attention-fmha-example/` | README.md, fmha_skeleton.md, tuning.md | complete | **Round 13**: H200-measured 88_hopper_fmha (376.1 TFLOPS ping-pong at B2 H16 S1024 D128). Build/run scripts + probe record. 41_fused fallback context documented. |
| `wiki/nvidia/code-walkthroughs/cutlass-cute/attention-fmha-collective/` | README.md, tuning.md | complete | **Round 13**: H200-measured collective config (348.5 TFLOPS ping-pong at B2 H16 S2048 D128 causal). Probe record with collective settings cited. |
| `wiki/nvidia/code-walkthroughs/flash-attention-v3/` | README.md, tuning.md | complete | **Round 1 closed**: Registered in MANIFEST.yaml (commit `bbda031f`, BSD-3-Clause, local path `{{FLASH_ATTENTION_REPO_REF}}`). Created README (source files, algorithmic comparison vs cutlass FMHA and MVP minimal) and tuning.md (compile-time + runtime knobs, differences from cutlass FMHA). |

---

## Section E — `sources/experience/hw-probes/` (Microbench records)

| Entry | Records | Classification | Notes / Gap |
|-------|---------|----------------|-------------|
| `sources/experience/hw-probes/wgmma-ptx/` | hello, zoo (11 configs) | complete | Already exercises the canonical zoo. AC-2.1 needs explicit 1:1 mapping table in `wiki/nvidia/hardware/wgmma/skill.md`, not new probe records. |
| `sources/experience/hw-probes/tma-ptx/` | hello, throughput, multicast-v1, multicast-v2 | complete | No gap. |
| `sources/experience/hw-probes/canonical/` | — (empty) | partial | Empty placeholder; clarify with maintainers whether this is a stub for future canonical microbenches or stale. Out of plan scope; flagged for visibility. |
| `sources/experience/hw-probes/attention-tuning/` | 2026-05-08-attention-tile-sweep.md, artifacts/profiles/ | complete | **Round 0 created**: 6-config BLOCK_M x BLOCK_N tile-shape sweep at fixed B=1,H=2,S=256,D=64 on H200. CSV includes throughput_gflops column. |
| `sources/experience/hw-probes/gemm-tuning/` (or operator-specific) | — | missing-or-deferred | AC-9 requires ≥1 measured sweep across operators; if attention-tuning is the chosen sweep, this stays optional. Plan lower bound permits one measured sweep total. |
...
## Section F — `sources/experience/api-probes/` (API-probe records)

| Entry | Files | Classification | Notes / Gap |
|-------|-------|----------------|-------------|
| `sources/experience/api-probes/gemm/` | 10 dated records, artifacts/ (14 .cu, build/run scripts, device.json, profiles/) | complete | Hot path; existing artifacts are the reference for AC-9 cross-linking. |
| `sources/experience/api-probes/attention/` | artifacts/{flash_attn_tma_wgmma.cu, flash_attn_minimal.cu, wgmma_desc_test.cu, cutlass_88_hopper_fmha.cu, build*.sh, run*.sh, device.json, profiles/}, 4 dated probe records | complete | **Round 14**: repo-local FMHA wrapper added. TMA+wgmma (0.35 TFLOPS), FMHA example (376.1 TFLOPS), FMHA collective (348.5 TFLOPS). All with H200 evidence. |
...
- `wiki/nvidia/foundations/compute/ilp/` missing apis.md.
- `wiki/nvidia/foundations/memory/bank-conflict/` missing apis.md.
- `wiki/nvidia/foundations/sync/memory-ordering/` is draft + spec-only.
- `sources/experience/hw-probes/canonical/` empty placeholder.
- `wiki/nvidia/api-definitions/{ptx,math-intrinsics,cccl/cub,_classified}/` empty namespaces.

### Reproducibility note
Re-running this audit against the current `` tree should reproduce the same classification provided no entries are added or moved. The classification uses observable filesystem state only (presence of `skill.md`, `pitfalls.md`, `tuning.md`, frontmatter `status`/`evidence_level`, hw-probe record counts) and verified upstream paths under `{{CUTLASS_REPO_REF}}/`.
