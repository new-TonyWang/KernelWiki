...
hardware:
  ...
  sm: 9.0a
source:
- path: 50-classical-algo/warp-specialization/skill.md
...
- path: 30-skill/compute/gemm-ptx/skill.md
  ...
  excerpt: TMA-PTX + wgmma-PTX composed into a single GEMM kernel; cutlass-free preprocessor
    + linked-binary gates
- path: 30-skill/compute/gemm/aligned/skill.md
...
- path: 80-experience/kernel-records/2026-04-29-gemm-ws-ptx/README.md
...
Q4. Is cutlass + cute available as a build-time and link-time dependency?
    YES --> CUTLASS-API track. See `30-skill/compute/gemm/aligned/skill.md`
            (aligned shapes) or `30-skill/compute/gemm/non-aligned-tail/skill.md`
            ...
            see `30-skill/compute/gemm-fused/cutlass-epilogue-prologue/skill.md`.
    ...
    NO  --> CUTLASS-FREE PTX track. The kernel must contain zero
            ...
            and linked-binary level. See `30-skill/compute/gemm-ptx/skill.md`
            for the single-tile composition; see
            `80-experience/kernel-records/2026-04-29-gemm-ws-ptx/README.md`
            ...
            algorithm skeleton in `50-classical-algo/warp-specialization`
...
The Hopper GEMM mainloop is a producer/consumer pipeline regardless of which track. Three variants from `50-classical-algo/warp-specialization/skill.md`:
...
The minimum-viable variant is **plain WS**. The cutlass-free working artifact in `80-experience/kernel-records/2026-04-29-gemm-ws-ptx/` realizes plain WS at one CTA + 5 warps (4 consumer + 1 producer); pingpong / cooperative are mechanical extensions of that scaffolding.
...
After each optimization, re-benchmark against `baseline` (cuBLASLt at the same shape) and follow the bottleneck triage in `70-reasoning/bottleneck-triage.md`.
...
- **Algorithm skeleton (variant-independent)**: `50-classical-algo/warp-specialization/skill.md`
- **Cutlass-free working kernel record**: `80-experience/kernel-records/2026-04-29-gemm-ws-ptx/`
