...
This file lists the optimization skills applicable to a custom reduction kernel, in recommended application order. Only skills that currently exist under `knowledge/30-skill/` (with a completed `skill.md`) are listed.
...
- **Specific guidance for reduction**:
  ...
  - When caching a multi-element-per-thread tile before reducing (Brent's theorem pattern), declare `__shared__ float tile[TILE][TILE+1]` if any column access is planned — measured on H200: unpadded `[32][32]` → 16.3 M bank conflicts, padded → 33 K (488× drop; see `80-experience/hw-probes/smem-tile-reuse/`).
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` -- "Block-level reduction without smem staging / with redundant global loads."

### 4. Barrier Optimization (scope narrowing + async-barrier overlap)

- **Skill path**: `30-skill/sync/barrier-optimization/`
- **Why it matters for reduction**: the block-level reduction tree uses `__syncthreads()` between warp-leader writes to smem and the final warp's read of those partials. On H200 sm_90a, each `__syncthreads()` costs ~31–62 ns depending on block size (measured: `80-experience/hw-probes/barrier-cost/`). For reductions that loop over many tiles with a `__syncthreads()` per tile, this cost compounds.
- **When to apply**:
...
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` — "Reduction kernel with `stall_barrier` above ~15 %."
...
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` — "Reduction-tail body too large for predication."
...
- **Measured impact**: 247.8× speedup of S1 over naive; 1498× for S1+grid-stride. See `80-experience/hw-probes/atomic-reduction-contention/2026-04-20-atomic-reduction.md`.
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` -- "many threads contending on the same atomic address" / high `stall_long_scoreboard` on `RED`/`ATOM`.
...
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` — multi-pass reductions where the second kernel's DRAM throughput stays at 100% Roofline even though the same buffer was just fully read by the first kernel (no L2 reuse).
...
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` — max/min reduction kernel where hot path shows `FSETP + FSEL` pairs instead of `FMNMX`. Correct response: replace ternary with `fmaxf`/`fminf`.
