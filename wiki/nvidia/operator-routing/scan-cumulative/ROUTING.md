...
This file lists the optimization skills applicable to a custom scan (prefix sum / cumulative) kernel, in recommended application order. Only skills that currently exist under `knowledge/30-skill/` (with a completed `skill.md`) are listed.
...
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` -- "Multi-pass block-level algorithm without smem working set."
...
- **Why it matters for scan**: block-level scan algorithms (Hillis- Steele, Blelloch) are inherently barrier-heavy — each iteration of the scan tree must `__syncthreads()` between read and write of the smem buffer. A 1024-element block scan with Blelloch requires `2 × log2(1024) = 20` `__syncthreads()` calls per phase (upsweep
  ...
  sm_90a at block size 1024, each `__syncthreads()` costs ~62 ns (measured: `80-experience/hw-probes/barrier-cost/`), so barrier cost alone is ~1.2 μs per block — often a noticeable fraction of total kernel time for scans.
- **When to apply**:
...
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` — "Scan kernel with `stall_barrier` > 20 %."
...
- **Relevance to bottleneck triage**: Q4b in `70-reasoning/bottleneck-triage.md` -- bytes-in-flight analysis.
...
- **Relevance to bottleneck triage**: Q4 in `70-reasoning/bottleneck-triage.md` — max-scan kernel showing `FSETP + FSEL` pairs instead of `FMNMX` in the shuffle-scan inner loop. Correct response: switch comparator to `fmaxf`.
