...
This file lists the optimization skills applicable to a custom indexing kernel, in recommended application order. Only skills that currently exist under `wiki/nvidia/foundations/` (with a completed `skill.md`) are listed.
...
- **Relevance to bottleneck triage**: Q4 in `reasoning/bottleneck-triage.md` — "Masked gather/scatter with non-predicable body on compute-bound kernel."
