...
TOOLS = [
        ...
        "inputSchema": {
            ...
            "properties": {
                "query": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Free-text keyword list, e.g. [\"flash\", \"attention\"]",
                },
                "type": {"type": "string", "description": "Filter by page type. Values: kernel, technique, hardware, pattern, language, migration, pr, blog, doc, contest, skill, experience, api-definition, operator-routing, algorithm, code-walkthrough, pitfall"},
                "tag": {"type": "string", "description": "Filter by tag (80+ tags). Examples: nvfp4, tcgen05, wgmma, tma, flash-attention. Supports aliases (UMMA -> tcgen05)"},
                "vendor": {"type": "string", "description": "Filter by vendor. Values: nvidia, ascend, biren, all. Auto-inferred from keywords/architecture when omitted"},
                "repo": {"type": "string", "description": "Filter by source repo (partial match). Examples: cutlass, sglang, vllm, flashinfer, pytorch, DeepGEMM"},
                "language": {"type": "string", "description": "Filter by DSL/language. Values: cuda-cpp, ptx, triton, cute-dsl, ascendc, triton-ascend, tilelang. Supports aliases (ascend c -> ascendc)"},
                "architecture": {"type": "string", "description": "Filter by architecture. Values: sm100, sm90, ascend910b, ascend910c. Supports aliases (B200 -> sm100, H100 -> sm90, 910B -> ascend910b)"},
                "symptom": {"type": "string", "description": "Filter by performance symptom (pattern pages). Values: low-sm-utilization, memory-bound, register-pressure, compute-bound, tail-effect, pipeline-stalls"},
                "confidence": {"type": "string", "description": "Filter by confidence level. Values: verified, source-reported, inferred, experimental"},
                "has_code": {"type": "boolean", "description": "Only return pages with associated source code artifacts", "default": False},
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "lookup": {"type": "string", "description": "Page id (e.g. kernel-flash-attention-4, pr-vllm-1234, hw-tcgen05, skill-ascend-vector-compute) or relative path (e.g. wiki/nvidia/kernels/flash-attention-4.md)"},
                "body_only": {"type": "boolean", "description": "Return only the markdown body text, without YAML frontmatter", "default": False},
                "frontmatter_only": {"type": "boolean", "description": "Return only the YAML frontmatter metadata", "default": False},
                "include_code": {"type": "boolean", "description": "Include artifact bundle files (code, diffs, kernel sources) associated with this page", "default": False},
                "follow_sources": {"type": "boolean", "description": "Include text excerpts from pages cited in the sources/source_refs fields", "default": False},
            },
            "required": ["lookup"],
        },
    },
